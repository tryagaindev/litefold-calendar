import { f as formatCalendarDate, g as parseCalendarDate, r as LitefoldCalendarError, t as createRegisteredExtension } from "../../chunks/registered-extension-contract-D6GEApjX.js";
//#region src/extensions/webmcp/configuration.ts
var DEFAULT_TOOL_NAME_PREFIX = "litefold-calendar";
var MAX_TOOL_NAME_PREFIX_LENGTH = 117;
var TOOL_NAME_PREFIX_PATTERN = /^[A-Za-z0-9_.-]+$/u;
var OPTION_KEYS = /* @__PURE__ */ new Set(["toolNamePrefix"]);
/** Validates and snapshots WebMCP configuration without consulting browser globals. */
function normalizeCalendarWebMcpOptions(options) {
	if (options === void 0) return Object.freeze({ toolNamePrefix: DEFAULT_TOOL_NAME_PREFIX });
	if (typeof options !== "object" || options === null || isArray(options)) throw createConfigurationError("webMcp options must be an object.");
	let keys;
	try {
	...
	if (typeof unknownKey === "string") throw createConfigurationError(`webMcp options.${unknownKey} is not a supported option.`);
	...
	const toolNamePrefix = configuredPrefix === void 0 ? DEFAULT_TOOL_NAME_PREFIX : configuredPrefix;
	if (typeof toolNamePrefix !== "string" || toolNamePrefix.length < 1 || toolNamePrefix.length > MAX_TOOL_NAME_PREFIX_LENGTH || !TOOL_NAME_PREFIX_PATTERN.test(toolNamePrefix)) throw createConfigurationError("webMcp options.toolNamePrefix must contain 1 through 117 ASCII letters, digits, periods, underscores, or hyphens.");
...
//#endregion
//#region src/extensions/webmcp/execution-signal.ts
function createExecutionSignalResolver(fallbackSignal) {
	const platform = createAbortSignalPlatform(fallbackSignal);
	const fallback = createExecutionSignal(fallbackSignal, platform);
	if (fallback === null) throw new TypeError("The extension lifecycle signal is not a usable AbortSignal.");
	return (options) => {
		try {
			return createExecutionSignal(isRecord$1(options) ? Reflect.get(options, "signal") : void 0, platform) ?? fallback;
		} catch {
			return fallback;
		}
	};
...
function createAbortSignalPlatform(referenceSignal) {
	...
	if (getAborted === null || addEventListener === null || removeEventListener === null) throw new TypeError("The extension lifecycle signal does not expose AbortSignal operations.");
...
function createExecutionSignal(value, platform) {
	...
	const isAborted = () => {
		...
		if (typeof aborted !== "boolean") throw new TypeError("AbortSignal.aborted did not return a boolean.");
		return aborted;
	};
	const addAbortListener = (listener) => {
		Reflect.apply(platform.addEventListener, value, [
			"abort",
			listener,
			{ once: true }
		]);
	...
	const removeAbortListener = (listener) => {
		Reflect.apply(platform.removeEventListener, value, ["abort", listener]);
	};
	try {
		isAborted();
...
function findPrototypeFunction(value, property, member) {
	...
	while (prototype !== null) {
		...
		const candidate = descriptor === void 0 ? void 0 : Reflect.get(descriptor, member);
...
//#endregion
//#region src/extensions/webmcp/runtime.ts
var DATE_INPUT_PATTERN = /^\d{4}-\d{2}-\d{2}$/u;
var GET_EVENTS_TOOL_SUFFIX = "-get-events";
var NAVIGATE_TOOL_SUFFIX = "-navigate";
var EVENT_PAGE_SIZE = 10;
var CURSOR_VERSION = 1;
var MAXIMUM_CURSOR_LENGTH = 512;
var CURSOR_PATTERN = /^lfc\.([0-9a-f]+)$/u;
var CURSOR_INSTANCE_ID_PATTERN = /^[A-Za-z0-9_-]{1,64}$/u;
var nextFallbackCursorInstance = 0;
var GET_EVENTS_INPUT_SCHEMA = Object.freeze({
	oneOf: Object.freeze([Object.freeze({
		additionalProperties: false,
		properties: Object.freeze({ date: Object.freeze({
		...
		additionalProperties: false,
		properties: Object.freeze({ cursor: Object.freeze({
...
var NAVIGATE_INPUT_SCHEMA = Object.freeze({
	oneOf: Object.freeze([Object.freeze({
		additionalProperties: false,
		...
		additionalProperties: false,
		properties: Object.freeze({ target: Object.freeze({ enum: Object.freeze([
			"today",
			"previous-month",
			"next-month"
		]) }) }),
...
var GET_EVENTS_ANNOTATIONS = Object.freeze({
	readOnlyHint: true,
	untrustedContentHint: true
});
var NAVIGATE_ANNOTATIONS = Object.freeze({ readOnlyHint: false });
/** Activates WebMCP against the exact capabilities declared by its extension definition. */
function activateWebMcp(context, toolNamePrefix) {
	const document = context.document;
	const navigation = context.navigation;
	const presentationEvents = context.presentationEvents;
	const state = context.state;
	if (document === void 0 || navigation === void 0 || presentationEvents === void 0 || state === void 0) throw new TypeError("The WebMCP extension did not receive its declared capabilities.");
...
var WebMcpController = class {
	...
	register(document) {
		...
		this.registerTools(modelContext).catch((cause) => {
			if (this.isDisposed || this.options.signal.aborted || !this.options.isLive()) return;
			this.options.fail(cause, "register");
		});
	}
	dispose() {
	...
	notifyStateChanged() {
		if (this.isDisposed || this.isStateNotificationPending) return;
	...
	executeGetEvents(input, signal) {
		if (isExecutionCanceled(signal)) return Promise.reject(createAbortError("The WebMCP tool execution was canceled."));
		const parsedInput = parseGetEventsInput(input);
		if (parsedInput === null) return Promise.resolve(this.createFailure("invalid-input", "Input must contain either an optional strict date or one opaque continuation cursor."));
		if (isExecutionCanceled(signal)) return Promise.reject(createAbortError("The WebMCP tool execution was canceled."));
		if (!this.isLive()) return Promise.resolve(this.createFailure("calendar-unavailable", "The calendar is no longer available."));
		const state = this.readState();
		return Promise.resolve(parsedInput.kind === "cursor" ? this.executeCursorGetEvents(parsedInput.cursor, state) : this.executeInitialGetEvents(parsedInput.date, state));
	}
	executeInitialGetEvents(date, state) {
		const dateString = date === null ? null : formatCalendarDate(date);
		if (state.range === null || dateString !== null && (dateString < state.range.start || dateString >= state.range.end)) return this.createFailure("date-outside-visible-range", state.range === null ? "The calendar does not have a visible range." : "The requested date is outside the current visible range; navigate the calendar first.");
		if (!this.options.state.hasCurrentSnapshot()) return this.createFailure(state.phase === "unavailable" ? "calendar-unavailable" : "date-not-loaded", state.phase === "unavailable" ? "The calendar has no usable event snapshot." : "The current visible range does not have a loaded event snapshot yet.");
	...
	executeCursorGetEvents(cursor, state) {
		if (!this.options.state.hasCurrentSnapshot()) return state.phase === "unavailable" ? this.createFailure("calendar-unavailable", "The calendar has no usable event snapshot.") : this.createPaginationStaleFailure();
		const token = parseCursorToken(cursor);
		if (token === null) return this.createFailure("invalid-input", "The pagination cursor is malformed.");
		if (token.instanceId !== this.options.cursorInstanceId) return this.createPaginationStaleFailure();
		const range = state.range;
		if (range?.start !== token.rangeStart || range.end !== token.rangeEnd || token.offset < EVENT_PAGE_SIZE || token.offset % EVENT_PAGE_SIZE !== 0 || token.dateString !== null && (token.dateString < token.rangeStart || token.dateString >= token.rangeEnd)) return this.createPaginationStaleFailure();
		const sourcePage = this.options.presentationEvents.getPage(token.date, token.offset, EVENT_PAGE_SIZE);
		if (sourcePage.snapshotRevision !== token.snapshotRevision || token.offset >= sourcePage.totalEvents) return this.createPaginationStaleFailure();
	...
	createVisibleEventsSuccess(sourcePage, dateString, offset, rangeStart, rangeEnd, state) {
		...
		const nextOffset = offset + page.length < totalEvents ? offset + page.length : null;
		const nextCursor = nextOffset === null ? null : createCursor(Object.freeze({
			dateString,
	...
	executeNavigate(input, signal) {
		...
		if (target === null) return Promise.resolve(this.createFailure("invalid-input", "Input must select a date, today, the previous month, or the next month."));
		if (!this.isLive()) return Promise.resolve(this.createFailure("calendar-unavailable", "The calendar is no longer available."));
		if (signal.isAborted()) return Promise.reject(createAbortError("The WebMCP tool execution was canceled."));
		...
		try {
			...
			if (this.navigationSequence === navigationSequence) this.navigationSequence = previousNavigationSequence;
			if (!this.isLive() || isExecutionCanceled(signal)) return Promise.reject(createAbortError("The calendar navigation was interrupted by teardown."));
	...
	waitForNavigation(commit, navigationSequence, signal) {
		if (signal.isAborted()) return Promise.reject(createAbortError("The WebMCP tool execution was canceled."));
		if (!this.isLive()) return Promise.reject(createAbortError("The calendar navigation was interrupted by teardown."));
		if (!commit.startedLoad) {
			if (navigationSequence !== this.navigationSequence || commit.navigationRevision !== this.options.state.getNavigationRevision()) return Promise.resolve(this.createFailure("navigation-superseded", "A newer calendar operation superseded this navigation."));
			const state = this.readState();
			if (!this.isLive()) return Promise.reject(createAbortError("The calendar navigation was interrupted by teardown."));
			if (state.phase === "unavailable") return Promise.resolve(this.createFailure("calendar-unavailable", "The calendar is not currently available."));
	...
	settleWaiter(waiter) {
		...
		let result = null;
		if (waiter.navigationSequence !== this.navigationSequence || waiter.navigationRevision !== this.options.state.getNavigationRevision() || this.options.state.getGeneration() !== waiter.generation) result = this.createFailure("navigation-superseded", "A newer calendar operation superseded this navigation.");
		else {
			const state = this.readState();
			if (state.phase === "unavailable") result = this.createFailure("calendar-unavailable", "The calendar could not load the requested view.");
			else if (state.phase !== "loading") result = Object.freeze({
	...
	readState() {
		if (this.isLive()) this.lastState = this.options.state.getState();
		return this.lastState;
	}
};
function parseGetEventsInput(input) {
	const keys = getInputKeys(input, /* @__PURE__ */ new Set(["cursor", "date"]));
	...
	if (keys.has("cursor")) {
		...
		return typeof cursor === "string" && cursor.length <= MAXIMUM_CURSOR_LENGTH && parseCursorToken(cursor) !== null ? Object.freeze({
			cursor,
			kind: "cursor"
		}) : null;
	}
	...
	try {
		if (keys.has("date")) {
			...
			if (typeof dateValue !== "string" || !DATE_INPUT_PATTERN.test(dateValue)) return null;
...
function parseCursorToken(cursor) {
	const encodedPayload = CURSOR_PATTERN.exec(cursor)?.[1];
	if (encodedPayload === void 0) return null;
...
function parseCursorPayload(serializedPayload) {
	...
	if (!Array.isArray(payload) || payload.length !== 8) return null;
	const values = payload;
	if (values[0] !== CURSOR_VERSION) return null;
	...
	if (instanceId === null || snapshotRevision === null || rangeStart === null || rangeEnd === null || scope === null || offset === null) return null;
	return Object.freeze({
		date: scope.date,
...
function parseStrictCursorDate(value) {
	...
	return date === null ? null : Object.freeze({
		date,
		dateString: value
	});
}
function parseCursorScope(scope, dateValue) {
	if (scope === "range") return dateValue === null ? Object.freeze({
		date: null,
		dateString: null
	}) : null;
...
function createCursor(payload) {
	const cursor = `lfc.${encodeAsciiHex(JSON.stringify([
		CURSOR_VERSION,
		payload.instanceId,
	...
	if (cursor.length > MAXIMUM_CURSOR_LENGTH) throw new RangeError("The WebMCP pagination cursor exceeded its internal bound.");
	return cursor;
}
function encodeAsciiHex(value) {
	let encoded = "";
	for (let index = 0; index < value.length; index += 1) encoded += value.charCodeAt(index).toString(16).padStart(2, "0");
	return encoded;
}
...
function parseNavigateInput(input) {
	const keys = getInputKeys(input, /* @__PURE__ */ new Set(["date", "target"]));
	if (keys?.has("target") !== true) return null;
	let target;
	try {
	...
	if (target === "date") {
		if (keys.size !== 2 || !keys.has("date")) return null;
		let dateValue;
		try {
	...
	if (keys.size !== 1 || target !== "today" && target !== "previous-month" && target !== "next-month") return null;
	...
	try {
		...
		if (crypto !== void 0) {
			...
			if (typeof randomUuid === "function") {
				...
				if (typeof value === "string" && CURSOR_INSTANCE_ID_PATTERN.test(value)) return value;
			}
			const getRandomValues = Reflect.get(crypto, "getRandomValues");
			if (typeof getRandomValues === "function") {
				const values = /* @__PURE__ */ new Uint32Array(4);
...
function getModelContext(document) {
	...
	if (candidate === void 0 || candidate === null || !isRecord(candidate)) return null;
	...
	return Object.freeze({ registerTool: (tool, options) => Reflect.apply(callable, candidate, [tool, options]) });
}
