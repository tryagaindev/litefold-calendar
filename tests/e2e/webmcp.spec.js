...
test("advanced example registers and invokes WebMCP tools with a browser mock", async ({ page }) => {
	...
	const invocation = await page.evaluate(async ({ getEventsToolName, navigateToolName }) => {
		...
		const continuationResult = await getEventsTool.execute({
	...
	expect(typeof invocation.rangeResult.nextCursor).toBe("string");
	...
	expect(invocation.continuationResult).toMatchObject({
		date: null,
		offset: 10,
		ok: true,
		totalEvents: 54
	});
	expect(typeof invocation.continuationResult.nextCursor).toBe("string");
	expect(invocation.continuationResult.events).toHaveLength(10);
