...
test.describe("large-text developer-page reflow", () => {
	...
	test("examples landing keeps long run titles inside their cards at 150% text", async ({ page }) => {
		await page.setViewportSize({ height: 844, width: 320 });
		const response = await page.goto("/examples/", { waitUntil: "domcontentloaded" });
		expect(response?.ok()).toBe(true);
		await expect(page.locator(".example-card")).toHaveCount(6);
		...
		await expect.poll(() => runLink.evaluate((element) => {
			...
			return element.getBoundingClientRect().right - card.getBoundingClientRect().right;
		})).toBeLessThanOrEqual(1);
		await expectNoDocumentHorizontalOverflow(page);
	});

	test("async-errors heading reflows inside the example shell at 200% text", async ({ page }) => {
		await page.setViewportSize({ height: 844, width: 320 });
		const response = await page.goto("/examples/async-errors/", { waitUntil: "domcontentloaded" });
		expect(response?.ok()).toBe(true);
		const heading = page.getByRole("heading", {
			exact: true,
			name: "Handle asynchronous failures"
		});
		...
		await expect.poll(() => heading.evaluate((element) => {
			...
			return element.getBoundingClientRect().right - shell.getBoundingClientRect().right;
		})).toBeLessThanOrEqual(1);
		await expectNoDocumentHorizontalOverflow(page);
	});
});
