...
Validated per-event `accentColor` values color only the built-in SVG marker. They do not tint event text, surfaces, borders, or the logical leading accent. Extension-owned category palettes must also include visible text or another non-color cue whenever the distinction matters.
...
The spacing rhythm is built from the YAML scale: `0.125rem`, `0.25rem`, `0.375rem`, `0.5rem`, `0.625rem`, `0.75rem`, `1rem`, `1.5rem`, and `2rem`. The public core gap is `--lfc-gap: 0.75rem`. Controls use `0.375rem` block and `0.625rem` inline padding with a `2.75rem` minimum. Grid events use `0.125rem` block and `0.25rem` inline padding with a `1.5rem` minimum block size. Wide day rows use `--lfc-day-min-block-size: clamp(3.5rem, 13cqi, 7rem)`, compact day rows use `--lfc-compact-day-min-block-size: 3.75rem`, and day padding uses `--lfc-day-padding: clamp(0.25rem, 1.5cqi, 0.75rem)`.

Package layout uses logical properties. Use `rem` for component layout and target size, `em` for type-relative details, and `cqi` for fluid container-relative sizing. The only pixel-unit exceptions are deliberate one-CSS-pixel hairlines and the canonical `1px` / `-1px` visually hidden sentinel; both are already constrained and documented by the repository's CSS conventions.
...
- **Above `42rem`:** Toolbar content stays on the primary row, weekday labels use the wide size, and every capped event action consumes the available day-cell width. A native grid-overflow action shows its localized package content or extension-supplied wide visual content without changing the action itself.
...
Grid events are compact lavender slips with a violet outline, magenta logical-leading rule, preserved marker slot, and ellipsized text slots. At compact widths the first action uses a marker-only presentation within the full compact target. A separate day-level cue uses two overlapping outlined cards in the event-border color to communicate plurality without an exact number. It sits on the marker's physical right side with clearance for noninteractive status decoration and centers in the summaries layer when no compact primary marker exists. When the native overflow action becomes the compact-primary control, the stack is hidden so the visible localized overflow count is not duplicated. Accessible naming and focus behavior follow the [interaction model](ACCESSIBILITY.md#interaction-model).

Wide grid overflow remains an explicit native count/action rather than silently dropping events. An extension may replace only its non-compact visual content; the canonical localized text remains in the button, and compact-primary or focused overflow actions continue to show it. Custom content cannot replace the action, accessible label, target geometry, or agenda-focus behavior.

The agenda is a rounded surface panel immediately below the grid. Its ordered event rows reuse the event palette and visually align leading content, localized time, title, supporting details, and application trailing content. The canonical action and extension semantics are defined by the [interaction model](ACCESSIBILITY.md#interaction-model) and [extension API](docs/api.md#extend-rendering-calendarextension).
...
These values keep the built-in pieces proportionate but are not public token promises: navigation prefers a `32rem` basis; the picker year track has a `7rem` minimum and `0.65fr` share; agenda titles use a `10rem` flex basis; and each pager lane is `min(6rem, 25cqi)`. Navigation glyphs occupy `1.25em`, status icons `1.5rem`, event markers `0.75em`, and compact markers `0.375rem`. Applications must not target private descendants to alter them.
...
- **Do** test theme overrides at narrow widths, 200% text size, 400% zoom, RTL, reduced motion, increased contrast, and forced colors. Include a generic `1.25rem` custom event marker with a noninteractive inline-end satellite to verify that the compact stack stays separate and causes no horizontal overflow.
...
- **Don't** treat application-owned example shells, dialogs, filters, or extension palettes as new package defaults.
