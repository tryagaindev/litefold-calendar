...
By default, the six week tracks have equal intrinsic height: every row matches the tallest week and the complete grid grows when content requires more space. In compact layout, this `"equal"` mode gives normally visible primary-event and compact-primary overflow roots a common full slot. Total-count buttons use the same width and a square minimum, with rounded borders, but may grow taller for wrapped or enlarged text. Later event summaries exposed only on focus remain intrinsically sized. `weekRowSizing: "content"` instead lets each week track size independently and keeps compact roots intrinsically sized without the count button's square minimum. Both modes preserve the target-size floor and allow count content to wrap and grow. Neither mode assigns a fixed calendar height or clips a busy week.
...
Previous/Next are transparent glyph controls with the standard component radius. Today is a quiet bordered control on the calendar background; its localized label may wrap within the available width. The interactive month title is centered and underlined. Eligible quiet controls preview the selected surface on hover-capable devices and use the same treatment on press. Disabled controls use `0.65` opacity without erasing their label; their operability and focus semantics follow the [interaction model](ACCESSIBILITY.md#interaction-model).
...
The following package custom properties are the stable bridge from an application design system to this design. The [Styling and customization](docs/styling.md) defines application, cascade, root-marker, and CSP mechanics. Only `.litefold-calendar` and these documented `--lfc-*` properties are public styling API; generated descendants, transient attributes, keyframes, pager internals, container names, and `--lfc-internal-*` properties remain private.
