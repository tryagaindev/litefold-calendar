...
export function createPackedExtensionTypeFixture(packageJson, factories) {
	...
	return [
		...
		"\ttype CalendarEventOverflowDefaultContext,",
		...
		"\t\tconst completion: CalendarEventOverflowDefaultContext<Metadata> = context;",
		"\t\tconst sourceId: string | undefined = completion.events[0]?.metadata?.sourceId;",
		"\t\tconst heading: HTMLHeadingElement = completion.agendaHeading;",
		"\t\tconst trigger: HTMLButtonElement = completion.triggerElement;",
		"\t\tvoid [sourceId, heading, trigger];",
		"\t}",
		"};",
