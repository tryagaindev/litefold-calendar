...
void test("draft assets and final release publication are digest-bound and source-free", async () => {
	...
	assert.match(verifyRegistry, /\(\.verified \| length\) == 1[\s\S]*?\.verified\[0\]\.name == "@tryagaindev\/litefold-calendar"[\s\S]*?\.verified\[0\]\.version == \$version/u);
	assert.match(verifyRegistry, /"\$\{alpha\}" == "\$\{LFC_VERSION\}"/u);
	assert.doesNotMatch(verifyRegistry, /actions\/checkout@|contents: write|id-token: write|npm publish/u);
