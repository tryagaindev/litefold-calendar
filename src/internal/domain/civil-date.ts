import type { CalendarDate, CalendarDateInput } from "../../types.js";
...
/** A local civil date/time parsed without applying a browser time zone. */
export interface CalendarDateTime extends CalendarDate {
	/** Fractional seconds represented as a seven-digit tick value. */
	readonly fractionalSecond: number;
	/** Whether the source contained a time component. */
...
export function parseCalendarDateTime(value: string): Readonly<CalendarDateTime> | null {
...
export function getCalendarDateForTimeZone(
	instant: Date,
	timeZone: string
): Readonly<CalendarDate> | null {
	const timeZoneCandidate: unknown = timeZone;
...
export function parseCalendarDate(value: CalendarDateInput): Readonly<CalendarDate> | null {
	...
	try {
		const date = {
			day: Reflect.get(value, "day") as unknown,
			month: Reflect.get(value, "month") as unknown,
			year: Reflect.get(value, "year") as unknown
		};
...
export function addCalendarDays(date: CalendarDate, days: number): Readonly<CalendarDate> {
...
export function addCalendarMonths(date: CalendarDate, months: number): Readonly<CalendarDate> {
	assertCalendarDate(date);
...
export function toCalendarDate(value: CalendarDate): Readonly<CalendarDate> {
...
export function toCalendarDateFromNow(now: Date): Readonly<CalendarDate> {
...
export function toUtcDateTime(date: CalendarDateTime): Date {
...
export function isMidnight(value: CalendarDateTime): boolean {
...
export function isValidCalendarDateTime(value: CalendarDateTime): boolean {
...
export function assertCalendarDate(date: CalendarDate): void {
...
function isValidFractionalSecond(value: CalendarDateTime): boolean {
...
function isZeroTime(value: CalendarDateTime): boolean {
	return value.hour === 0 && value.minute === 0 && value.second === 0 && value.fractionalSecond === 0;
}

function assertCalendarDateTime(value: CalendarDateTime): void {
...
function fromUtcDayNumber(dayNumber: number): Readonly<CalendarDate> {
...
function hasNumericCalendarFields(value: Readonly<Record<"day" | "month" | "year", unknown>>): value is CalendarDate {
