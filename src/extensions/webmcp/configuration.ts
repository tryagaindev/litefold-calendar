...
const OPTION_KEYS: ReadonlySet<string> = new Set(["toolNamePrefix"]);

/** Configuration for the optional first-party WebMCP extension. */
export interface CalendarWebMcpOptions {
	/** Prefix used for the `-get-events` and `-navigate` tool names. */
	readonly toolNamePrefix?: string;
}

interface NormalizedCalendarWebMcpOptions {
	readonly toolNamePrefix: string;
}

/** Validates and snapshots WebMCP configuration without consulting browser globals. */
export function normalizeCalendarWebMcpOptions(
	options: unknown
): Readonly<NormalizedCalendarWebMcpOptions> {
	...
	if (typeof toolNamePrefix !== "string" || toolNamePrefix.length < 1 ||
		...
		throw createConfigurationError(
			"webMcp options.toolNamePrefix must contain 1 through 117 ASCII letters, digits, periods, underscores, or hyphens."
		);
	}
	return Object.freeze({ toolNamePrefix });
}

function createConfigurationError(message: string, cause?: unknown): LitefoldCalendarError {
