...
Where Popover is unavailable, the same picker uses a native modal `<dialog>` to meet the [Baseline Widely available policy](docs/browser-support.md). Native modality keeps focus within the picker and makes the rest of the document inert. Jump, Cancel, Escape, and backdrop dismissal restore the trigger; invalid input remains open. Theme changes preserve the open surface, field values, selection, and focus in both implementations. Test the fallback independently by disabling the optional capability; current-engine success alone does not verify it.
...
Toolbar controls remain in the same DOM and sequential-focus order at every width: Previous, Next, the interactive month title, Today, then application `toolbarEnd` content. The [responsive composition](DESIGN.md#responsive-model) changes rows without visual reordering. Below a `24rem` calendar content width, CSS exposes the already-rendered locale-aware `month: "short", year: "numeric"` visual; at exactly `24rem` and above, it exposes the full-month visual. The canonical full title text, trigger `aria-label`, grid accessible name, and live announcement remain complete. The compact title span is `aria-hidden` and cannot duplicate them. If a localized title still cannot fit, only its visual presentation ellipsizes. Container resizing changes only CSS layout and does not rerender, replace focused nodes, refetch events, rerun render hooks, re-enter extension lifecycle, or require focus restoration.
...
- Each decorative lane follows the toolbar's full versus abbreviated month/year presentation at the `24rem` boundary, but the lane and both of its text variants remain `aria-hidden`.
...
- Keep render-hook output concise and noninteractive; nested controls do not belong inside event actions. Fit custom compact visuals within their assigned slot, or increase `--lfc-control-min-size` and `--lfc-grid-event-min-block-size` so equal package-owned roots grow together. Oversized output remains visible but is outside the equal-sizing guarantee.
...
2. **Picker and bounds**
   - Open the month/year picker with native Popover enabled, then repeat with that capability disabled to exercise the modal dialog fallback. Exercise validation, Jump, Cancel, Escape, and pointer light-dismiss. Confirm outside popover dismissal preserves the user's new focus target, while modal backdrop dismissal restores the trigger and releases document inertness.
...
3. **Selection, events, and replacement**
   ...
   - For zero-, one-, and multiple-event days, verify the compact default exposes a total-count button only for multiple occurrences. Repeat `"count"` and `"count-when-multiple"` at compact and wide widths, including static events, suppressed markers, and `maxGridEventsPerDay: 0`. Confirm the exact total and date remain in each action's accessible name, count presentation excludes individual summaries from new action entry, and default activation focuses the selected-day agenda without invoking `onDaySelect`.
   - Exercise `onEventOverflowActivate` with and without synchronous cancellation. Confirm its occurrence snapshot is complete, cancellation lets the application own its chooser and focus restoration, and callback navigation or destruction prevents stale default selection.
   - Explicitly set `gridEventDisplay.compact: "events"` to verify the passive cue: with a positive summary cap it is absent for zero and one, reports signed additional occurrences with a primary marker, and reports the unsigned total when no marker is visible. Confirm the cue remains decorative and pointer-transparent while the day button announces the exact total. With `maxGridEventsPerDay: 0`, confirm the numeric fallback stays inside one native overflow action for every nonempty day and preserves managed keyboard and agenda behavior.
