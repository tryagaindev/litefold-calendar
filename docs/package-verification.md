...
`devEngines` is manifest-only npm CLI policy. The root package descriptor in
`package-lock.json` intentionally does not duplicate it, which keeps lockfiles
generated by npm and Dependabot canonical while `package.json` remains the
single source for contributor tool requirements.
