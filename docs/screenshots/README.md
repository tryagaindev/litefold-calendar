...
| `mobile-month-agenda-dark-390x844.png` | Compact dark navigation, month grid, and selected-day agenda | 390 × 844 |
...
Review each image at native dimensions. Confirm focus, selection, Today, overflow, compact navigation, one-line month title, event marker, agenda, dialog, and month-picker states match their scene. In the settled 390px scene, verify the built-in controls share their compact row above application toolbar content and that no pager lane, offset, motion residue, or horizontal scrollbar remains visible. In the held-pull scene, verify only one live grid is present and the exposed lane shows the correct arrow and localized adjacent month without event content. Inspect for clipped text, system-font drift, unexpected network content, and personal or secret data.
