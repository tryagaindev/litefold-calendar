...
litefold-calendar has two primary risk centers: untrusted event data crossing into an application's DOM and navigation surface, and repository changes crossing into a public npm artifact.  Runtime controls emphasize whole-snapshot validation, non-executable rendering, restricted links, bounded work, async generation guards, lifecycle isolation, and visible failures.  Supply-chain controls emphasize read-only pull-request CI, exact dependencies, clean committed source, a protected alpha tag, immutable artifact receipts, isolated npm OIDC authority, and publication of the exact verified tarball with provenance.
...
| Source, tarball, receipts, and npm identity | Publish only reviewed bytes from the intended repository and commit |
| GitHub/npm maintainer authority | Keep write and OIDC privileges away from untrusted code |
...
| TM-006 | Pull-request code gains release authority or weakens package policy | Read-only workflow defaults, full-SHA actions, no `pull_request_target`, script-disabled locked installs, CodeQL/dependency review, source/output policy | Hosted branch/ruleset configuration and independent approval are external |
...
| `src/styles.css` | Scoped styling, responsive/forced-color behavior, no dynamic style sink |
...
Every security-relevant change should preserve hostile-input, async race, teardown, cross-instance, safe-link, error-presentation, package-policy, clean-consumer, and workflow assertions.  External GitHub and npm settings must be checked manually before release and after any ownership, repository-name, or workflow-file change.
