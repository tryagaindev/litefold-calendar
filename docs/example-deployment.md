...
- Injects the current self-only Content Security Policy, including `worker-src 'none'`, and developer provenance navigation into every newly staged page.
...
- Accepts only the exact current policy or the exact previous `worker-src 'self'` policy on retained HTML, preserving historical bytes while rejecting every other policy variation.
...
Manual rollback transfers no producer-composed site artifact. Its writer authenticates the current `pages-content` head and selected retained ancestor, extracts the selected commit's exact `main/` bytes, preserves the current retained releases and root shell, and runs the trusted default-branch assembler as a credential-stripped unprivileged child. The assembler requires one recognized exact Content Security Policy before runtime resources and rejects remote root runtime assets. Exact reruns, older release backfills, and rollbacks may preserve the previous `worker-src 'self'` policy; a normal descendant `main` deployment or newer release upgrades the root shell to `worker-src 'none'`. A missing, renderer-incompatible, or unknown-policy retained shell fails closed when it cannot be safely replaced; do not substitute raw repository shell files.
