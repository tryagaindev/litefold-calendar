...
This repository-only record explains why the API is narrow and preserves the evidence used to choose it. The [API reference](api.md#control-the-calendar-calendar) is authoritative for current signatures and behavior if the implementation has evolved since this decision.
...
`createCalendar<TMetadata>()` returns `Calendar<TMetadata>`. `setEvents()` replaces the complete static snapshot or provider, then resolves the current visible range on the existing instance. A direct array finishes before the method returns; a PromiseLike keeps the loading-then-terminal lifecycle. The method does not update locale, time zone, date bounds, callbacks, render hooks, complete extensions, messages, icons, limits, or integration nodes. Those values remain construction-time configuration and require recreation.
...
| Change | Operation |
|---|---|
| Replace the complete static event array or provider | `setEvents()` |
| Refresh the current provider after a filter, cache, or authorization change | Update application state, then call `refetchEvents()` |
| Change locale, time zone, bounds, callbacks, extensions, render hooks, messages, icons, limits, or integration nodes | Destroy and recreate the calendar |
...
| Progressive enhancement | Keep fallback markup outside the host; replace data while a usable snapshot exists; recreate only for construction-time changes | Replacement keeps usable content visible. Recreation exposes pending fallback only when the new source returns a PromiseLike; a direct result coordinates fallback before `render()` returns. |

The fixtures also establish these behavior differences:

- Replacing a static array through provider state works, but still requires the application to snapshot the correct source and filter for each request. The provider may return that snapshot directly for an immediate commit or wrap it in `Promise.resolve()` to opt into loading behavior.
...
- Refetch aborts the prior request and ignores its late result. Recreation also aborts through `destroy()`, but additionally replaces all DOM, listeners, render-hook mounts, extension lifecycles, integration-node leases, and observable instance identity.

## Alternatives

### Recreate the instance

Recreation remains the correct operation when construction-time configuration changes. It provides a clean ownership boundary and already aborts source, render-hook, and extension work. It is a poor default for event replacement because it clears the usable snapshot, resets agenda paging, replaces focused nodes, and reruns every render-hook mount and extension lifecycle. A PromiseLike source also temporarily returns progressive enhancement to pending fallback state; a direct result settles fallback before `render()` returns.
...
The tradeoff is a public and internal contract change. `Calendar<TMetadata>` preserves the metadata type selected by `CalendarOptions<TMetadata>`, and the coordinator stores a safely replaceable current event array or provider.
...
The coordinator stores a mutable current `CalendarEvents<TMetadata>` separate from the immutable options snapshot, preserving a static array or provider without wrapping one as the other. A focused source module returns a private synchronous/asynchronous discriminant for every request, while shared terminal helpers preserve generation, error, focus, agenda, and fallback behavior. Argument resolution reuses the hardened array inspection used at construction while mapping method misuse to `invalid-argument`.
...
The decision record, integration fixtures, and measurement harness remain repository-only artifacts; the package manifest publishes `dist/` alone.

## Measurement protocol

The measurement commands are opt-in and are useful when a change may affect rendering, event replacement, library loading, or emitted size. Their elapsed times and byte counts are diagnostic; they are not repository thresholds.
...
Use the broader workflow harness to measure the root package import, initialization, a dense initial render, next-month navigation, direct `setEvents()`, and controlled PromiseLike start and completion:

```sh
npm run measure:workflows
```

It reports fresh-process import samples, only the modules reachable from the root entry (excluding optional WebMCP), public stylesheet raw/gzip bytes, render mutations, and median/p95 JSDOM timings. It deterministically asserts source calls, state phases, grid commits, and dense DOM limits; these operation counts catch redundant renders without relying on machine speed. Pass `--runs N`, `--warmups N`, `--import-runs N`, `--import-warmups N`, or `--json` after `--` to adjust diagnostic sampling. The fast tooling tests validate the import-graph, stylesheet-size, and operation-count contracts; the timed workflow remains opt-in.

The npm commands build first, so their build messages precede a JSON report. For clean machine-readable workflow output, build once and invoke the measurement script directly:
...
Record Node, operating system, architecture, sample counts, median, p95, raw bytes, gzip bytes, and baseline identity when sharing results. Compare runs only when build, runtime, hardware, and protocol match. Historical machine-specific numbers are deliberately not retained as a current performance claim; run the relevant command against the code under review.
